    ...
    def loss(self,
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                losses['loss_cls'] = 0.1 * self.loss_cls(
                    cls_score,
                    labels,
                    label_weights,
...
#        print(cls_score,'               aaaaaaaaaaaaaaaaaaaaaaaaaaaaa')

        scores = F.softmax(
            cls_score, dim=-1) if cls_score is not None else None
#        print(scores,'               bbbbbbbbbbbbbbbbbbbbbbbbbbbbb')
...
#            print(score,'             ccccccccccccccccccccccccccc')
            if cfg is not None:
...
#        losses = dict()
#        num_pos = pos_bboxes.size(0)
#        norm_logstd = mean_norm_logstd[:num_pos]
#        losses['loss_kl'] = self.kl_loss(pos_bboxes,pos_gt_bboxes,logstd=norm_logstd)
#        
#        
#        if cls_score is not None:
#            avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
...
#        
#        
#    def kl_loss(self,pred, target, logstd=None, delta=1.414, eps=1e-4):
...
#        klloss = torch.where(diff_weighted < delta,
#                             0.5 * ((diff_weighted)**2),
#                             delta * (diff_weighted - 0.5 * delta)) + logstd
#        kl_loss = klloss.sum(dim=1)
#        kl_loss = torch.exp(-0.1 * kl_loss)
#        # if training:
#        #     mean_inv_std *= 1 - momentum
#        #     mean_inv_std += momentum * torch.mean(inverse_std.detach())
#        # klloss.div_(mean_inv_std.clamp(min=1e-6))
#        return kl_loss
#
#    @force_fp32(apply_to=('cls_score', 'bbox_pred'))
#    def get_bboxes(self,
...
#        scores = F.softmax(
#            cls_score, dim=-1) if cls_score is not None else None
#        print(scores.shape,'               aaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
#
#        batch_mode = True
#        if rois.ndim == 2:
#            # e.g. AugTest, Cascade R-CNN, HTC, SCNet...
